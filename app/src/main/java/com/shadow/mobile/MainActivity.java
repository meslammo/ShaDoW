...
/** MOD-23.2: JARVIS shell with offline conversation fallback and guaranteed local TTS path. */
public class MainActivity extends Activity implements TextToSpeech.OnInitListener {
    ...
    private void execute(){String s=input.getText().toString().trim();if(s.isEmpty())return;addUser(s);input.setText("");status.setText("THINKING • JARVIS");new Thread(()->{String r;try{r=handleUnified(s);}catch(Throwable e){r=core.offlineChat(s);}String out=r;runOnUiThread(()->{addAssistant(out);status.setText("ONLINE • JARVIS MODE");speak(out);});}).start();}
    private String handleUnified(String request){String local=core.handle(request);if(isDefinitiveLocal(local))return local;String answer=python.handle(request);if(answer!=null&&!answer.trim().isEmpty()&&!isExternalOffline(answer))return answer;return core.offlineChat(request);}
    private boolean isExternalOffline(String s){String x=s.toLowerCase(Locale.ROOT);return x.contains("غير متصل")||x.contains("المحرك الخارجي")||x.contains("external ai")||x.contains("offline-safe mode")||x.contains("configured ai provider")||x.contains("محرك الذكاء الخارجي");}
    private boolean isDefinitiveLocal(String s){return s!=null&&(s.startsWith("SHADOW ACTION")||s.startsWith("CALCULATOR")||s.startsWith("SHADOW SYSTEM STATUS")||s.startsWith("HOME CORE")||s.startsWith("CAR CORE")||s.startsWith("DEVICE")||s.startsWith("LOCAL MEMORY")||s.startsWith("SHADOW LOCAL CHAT")||s.startsWith("SHADOW\n\nأهلاً")||s.startsWith("SHADOW\n\nالوقت")||s.startsWith("SHADOW\n\nالتاريخ"));}
