...
public class JarvisMainActivity extends Activity implements TextToSpeech.OnInitListener {
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b);getWindow().setStatusBarColor(BG);getWindow().setNavigationBarColor(BG);core=new ShadowCore(this);cloud=new ShadowCloudClient(this);githubAuth=new ShadowGithubAuth(this);orchestrator=new ShadowMasterOrchestrator();identity=new ShadowVoiceIdentityGateway(this);remotes=null;developmentAgent=new ShadowDevelopmentAgent(this);pythonBridge=new ShadowPythonRuntimeBridge(this);spatialRadar=null;masterCycle=new ShadowMasterCycle();companionRegistry=null;verificationLedger=new ShadowVerificationLedger(this);recoveryLedger=new ShadowRecoveryLedger(this);lifecycleBridge=new ShadowMasterLifecycleBridge(orchestrator.events(),core,developmentAgent,null,pythonBridge,null,verificationLedger,recoveryLedger,masterCycle);
        ...
        tts=new TextToSpeech(this,this);if(isWakeEnabled())try{ShadowWakeWordService.start(this);}catch(Exception ignored){}bargeInMonitor=new ShadowBargeInMonitor(this,new ShadowBargeInMonitor.Listener(){public void onSpeechDetected(){runOnUiThread(()->{if(voiceState!=null&&voiceState.getState()==ShadowVoiceStateMachine.State.BOT_SPEAKING){voiceState.speechStarted();voiceHandler.postDelayed(()->{if(voiceState!=null&&voiceState.getState()==ShadowVoiceStateMachine.State.LISTENING&&!isFinishing()&&!voice.isActive())listen();},100);}});}public void onLevel(float level){runOnUiThread(()->{if(liveVoice!=null&&voiceState!=null&&voiceState.getState()==ShadowVoiceStateMachine.State.BOT_SPEAKING){liveVoice.setAlpha(.55f+.45f*level);}});}});ui();check();}
    ...
    private void features(){
        ...
        String[] a={
            "🌐 Web search","🎨 Image design","⚡ Full 35-Phase Master",
            "🧠 Development Agent","🛰 Master Bus","🧩 35-Phase Platform",
            "🩺 Shadow Diagnostics","🧠 Think Hard"+(reasoningMode?" ✓":""),
            "🔴 Deep Think"+("xhigh".equals(reasoningEffort)?" ✓":""),
        ...
        p.setOnMenuItemClickListener(i->{
            String n=i.getTitle().toString();
            if(n.startsWith("⚡ Full 35-Phase Master"))runFullMasterPipeline();
            else if(n.startsWith("🛰"))assistant(lifecycleBridge==null?"Master Bus غير متاح.":lifecycleBridge.status());
            else if(n.startsWith("🧩")){new Thread(()->{try{String statusText=pythonBridge.platformStatus();runOnUiThread(()->assistant(statusText));}catch(Throwable e){runOnUiThread(()->assistant("تعذر قراءة عقد الـ35 مرحلة: "+e.getMessage()));}}).start();}
            else if(n.startsWith("🩺")){new Thread(()->{try{String statusText=pythonBridge.platformDiagnostics();runOnUiThread(()->assistant(statusText));}catch(Throwable e){runOnUiThread(()->assistant("تعذر تشغيل تشخيص Shadow: "+e.getMessage()));}}).start();}
            else if(n.startsWith("🌐")){input.setText("ابحث أونلاين عن ");input.requestFocus();}
            else if(n.startsWith("🎨")){input.setText("صمم صورة: ");input.requestFocus();}
            else if(n.startsWith("🧠 Think Hard")){setReasoning(true,"high");assistant("🧠 Think Hard اتفعل — Black/Red mode.");}
            else if(n.startsWith("🔴 Deep Think")){setReasoning(true,"xhigh");assistant("🔴 Deep Think اتفعل — Black/Red mode.");}
            else if(n.startsWith("🛑 Stop Deep Think")){setReasoning(false,"none");assistant("تم إيقاف Think Hard / Deep Think.");}
            else if(n.startsWith("🧠 Development"))system("Development Agent: analyze → plan → approve → edit → test → report");
            else if(n.startsWith("🔐 GitHub"))connectGithub();
    ...
    private void runFullMasterPipeline(){
        ...
        final String task=request.isEmpty()?"شغّل مسار SHADOW الموحد واختبر عقد الـ35 مرحلة":"شغّل مسار SHADOW الموحد: "+request;
    ...
    @Override protected void onDestroy(){cleanupVoiceprint();handsFreeVoice=false;voiceHandler.removeCallbacksAndMessages(null);if(voiceState!=null)voiceState.setHandsFree(false);stopBargeInMonitor();if(bargeInMonitor!=null)bargeInMonitor.destroy();if(voice!=null)voice.destroy();stopCurrentTts();if(tts!=null)tts.shutdown();if(lifecycleBridge!=null)lifecycleBridge.destroy();super.onDestroy();}
}
