...
public class JarvisMainActivity extends Activity implements TextToSpeech.OnInitListener {
    ...
    private LinearLayout rootView; private TextView plusButtonRef,sendButtonRef,micButtonRef,menuButtonRef,moreButtonRef;
    ...
    private void setReasoningTheme(boolean on){reasoningMode=on;if(rootView==null)return;rootView.setBackgroundColor(on?REASON_BLACK:BG);if(plusButtonRef!=null)plusButtonRef.setTextColor(on?REASON_RED:TEXT);if(sendButtonRef!=null)sendButtonRef.setTextColor(on?REASON_RED:TEXT);if(micButtonRef!=null)micButtonRef.setTextColor(on?REASON_RED:TEXT);if(menuButtonRef!=null)menuButtonRef.setTextColor(on?REASON_RED:TEXT);if(moreButtonRef!=null)moreButtonRef.setTextColor(on?REASON_RED:TEXT);stage(on?"deep thinking":"online");}
    ...
    private void send(){String s=input.getText().toString().trim();if(s.isEmpty())return;user(s);input.setText("");
        ...
        stage("online");new Thread(()->{try{ShadowCloudClient.CloudReply r=cloud.chat(request, reasoningEffort);cloudOnline=true;runOnUiThread(()->{stage("analyzing");if(r.pendingAction!=null){if(!r.answer.isEmpty())assistant(r.answer);handlePendingAction(request,r);return;}assistant(r.answer);speak(r.answer);stage(r.offline?"reconnecting":"online");});}catch(Throwable cloudError){cloudOnline=false;String local=null;try{local=core.handle(s,identity.isAuthenticated(),true,"android-master-orchestrator");}catch(Throwable ignored){}if(local==null||local.trim().isEmpty())try{local=core.offlineChat(request);}catch(Throwable ignored){}final String answer=(local==null||local.trim().isEmpty())?"الأونلاين مش متاح دلوقتي، ومفيش إجابة آمنة متاحة للأمر.":local;runOnUiThread(()->{assistant(answer);stage("reconnecting");speak(answer);});}}).start();}
