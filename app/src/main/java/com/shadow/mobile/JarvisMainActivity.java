...
public class JarvisMainActivity extends Activity implements TextToSpeech.OnInitListener {
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b);getWindow().setStatusBarColor(BG);getWindow().setNavigationBarColor(BG);core=new ShadowCore(this);cloud=new ShadowCloudClient(this);githubAuth=new ShadowGithubAuth(this);orchestrator=new ShadowMasterOrchestrator();identity=new ShadowVoiceIdentityGateway(this);remotes=new ShadowRemoteController(this);developmentAgent=new ShadowDevelopmentAgent(this);pythonBridge=new ShadowPythonRuntimeBridge(this);spatialRadar=new ShadowRadarController(this,orchestrator.events());companionRegistry=new ShadowCompanionRegistry(this);verificationLedger=new ShadowVerificationLedger(this);recoveryLedger=new ShadowRecoveryLedger(this);lifecycleBridge=new ShadowMasterLifecycleBridge(orchestrator.events(),core,developmentAgent,remotes,pythonBridge,companionRegistry,verificationLedger,recoveryLedger);
    ...
    private void send(){String s=input.getText().toString().trim();if(s.isEmpty())return;user(s);input.setText("");
        ...
        // CHAT route: online-only AI path. There is no offline AI fallback.
        stage("online");new Thread(()->{try{ShadowCloudClient.CloudReply r=cloud.chat(request, reasoningEffort);cloudOnline=true;runOnUiThread(()->{stage("analyzing");if(r.pendingAction!=null){if(!r.answer.isEmpty())assistant(r.answer);masterEvent(ShadowMasterEventBus.Type.ACTION_REQUESTED,request,plan.routeName(),"online_action_pending",true);handlePendingAction(request,r);return;}masterEvent(ShadowMasterEventBus.Type.VERIFICATION_RESULT,request,plan.routeName(),"online_response_received",!r.answer.isEmpty());masterEvent(ShadowMasterEventBus.Type.MEMORY_WRITE,request,plan.routeName(),"conversation_response",true);masterEvent(ShadowMasterEventBus.Type.COMPLETED,request,plan.routeName(),"online_chat_completed",true);assistant(r.answer);speak(r.answer);stage("online");});}catch(Throwable cloudError){cloudOnline=false;String message=String.valueOf(cloudError.getMessage()==null?"online_service_unavailable":cloudError.getMessage());runOnUiThread(()->{masterEvent(ShadowMasterEventBus.Type.FAILED,request,plan.routeName(),"online_agent_unavailable:"+message,true);assistant("الأونلاين مش متاح دلوقتي، وSHADOW مش هيستخدم نسخة أوفلاين أو إجابة محلية بدل الذكاء السحابي.");stage("reconnecting");});}}).start();}
