...
/** MOD-29.4: Cloud gateway for chat, web-enabled answers and image generation. */
public final class ShadowCloudClient {
    ...
    public CloudReply chat(String message) throws Exception {
        ...
        try {
            ...
            c.setRequestMethod("POST"); c.setDoOutput(true); c.setConnectTimeout(8000); c.setReadTimeout(60000);
            ...
            if (code < 200 || code >= 300 || !result.optBoolean("ok", false)) throw new IllegalStateException(result.optString("error", "cloud_request_failed"));
    ...
    public String generateImage(String prompt) throws Exception {
        if (!isConfigured()) throw new IllegalStateException("Cloud backend is not configured");
        JSONObject body = new JSONObject(); body.put("prompt", prompt); body.put("size", "1024x1024");
        HttpURLConnection c = null;
        try {
            c = (HttpURLConnection) new URL(baseUrl + "/v1/images").openConnection();
            c.setRequestMethod("POST"); c.setDoOutput(true); c.setConnectTimeout(10000); c.setReadTimeout(120000);
            c.setRequestProperty("Content-Type", "application/json; charset=utf-8"); c.setRequestProperty("Accept", "application/json");
            byte[] bytes = body.toString().getBytes(StandardCharsets.UTF_8); c.setFixedLengthStreamingMode(bytes.length);
            try (OutputStream out = c.getOutputStream()) { out.write(bytes); }
            int code = c.getResponseCode();
            ...
            JSONObject result = new JSONObject(json == null ? "{}" : json);
            if (code < 200 || code >= 300 || !result.optBoolean("ok", false)) throw new IllegalStateException(result.optString("error", "image_generation_failed"));
    ...
    private String deviceProfile() {
        ...
        try {
            ...
            d.put("android", android.os.Build.VERSION.RELEASE);
    ...
    private static String read(InputStream stream) throws Exception {
        ...
        try (BufferedReader r = new BufferedReader(new InputStreamReader(stream, StandardCharsets.UTF_8))) { String line; while ((line = r.readLine()) != null) b.append(line); }
        return b.toString();
    }

    public static final class CloudReply {
        public final String answer; public final String responseId;
