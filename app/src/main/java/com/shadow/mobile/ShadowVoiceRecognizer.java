...
/** MOD-29.1: In-app turn-by-turn speech recognition; never launches the Google voice dialog. */
public final class ShadowVoiceRecognizer {
    ...
    public boolean isOnDeviceAvailable() {
        return android.os.Build.VERSION.SDK_INT >= 31 && SpeechRecognizer.isOnDeviceRecognitionAvailable(context);
    }
    ...
    public void start(String language) {
        ...
        stop();
        try {
            recognizer = (android.os.Build.VERSION.SDK_INT >= 31 && isOnDeviceAvailable())
                    ? SpeechRecognizer.createOnDeviceSpeechRecognizer(context)
                    : SpeechRecognizer.createSpeechRecognizer(context);
            recognizer.setRecognitionListener(new RecognitionListener() {
                @Override public void onReadyForSpeech(Bundle params) { active = true; listener.onListening(); }
                @Override public void onBeginningOfSpeech() { }
                @Override public void onRmsChanged(float rmsdB) { }
                @Override public void onBufferReceived(byte[] buffer) { }
                @Override public void onEndOfSpeech() { active = false; }
                @Override public void onError(int error) {
                    active = false;
                    if (error == SpeechRecognizer.ERROR_NO_MATCH || error == SpeechRecognizer.ERROR_SPEECH_TIMEOUT) {
                        listener.onError("مش سامع كلام واضح — قولها تاني.");
                    } else {
                        listener.onError("حصل خطأ في التعرف على الصوت (" + error + ").");
                    }
                }
                @Override public void onResults(Bundle results) {
                    active = false;
                    ArrayList<String> values = results == null ? null : results.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION);
                    if (values != null && !values.isEmpty() && values.get(0) != null && !values.get(0).trim().isEmpty()) listener.onText(values.get(0).trim());
                    else listener.onError("مش سامع كلام واضح — قولها تاني.");
                }
