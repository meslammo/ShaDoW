...
/** MOD-42.1: independent camera data + detection engine using OSM/Overpass. */
public final class ShadowRadarManager {
    ...
    private void saveCache(){try{JSONArray a=new JSONArray();for(Camera c:cameras){JSONObject o=new JSONObject();o.put("lat",c.lat);o.put("lon",c.lon);if(c.limit!=null&&!Double.isNaN(c.limit.doubleValue()))o.put("limit",c.limit.doubleValue());a.put(o);}write(cache(),a.toString());}catch(Exception ignored){}}
