...
class UnifiedRuntime:
    ...
    def execute(self, online_fn, offline_fn):
        ...
        try:
            value = online_fn(); self.mode, self.last_error = "online", None
            self.last_success = time.time(); self.last_online_probe = self.last_success
            if was_offline: self.recovery_count += 1
            return value
        except Exception as exc:
            self.last_error = f"{type(exc).__name__}: {exc}"[:512]
            self.mode = "offline"; self.fallback_count += 1; self.last_online_probe = time.time()
            value = offline_fn(); self.last_success = time.time(); return value
    def probe(self, online_fn) -> bool:
        self.last_online_probe = time.time()
        try:
            online_fn(); was_offline = self.mode == "offline"
            self.mode, self.last_error = "online", None
            if was_offline: self.recovery_count += 1
            return True
        except Exception as exc:
            self.mode = "offline"; self.last_error = f"{type(exc).__name__}: {exc}"[:512]; return False
    def status(self) -> Dict[str, Any]:
        return {"mode": self.mode, "last_error": self.last_error, "last_success": self.last_success,
                "last_online_probe": self.last_online_probe, "fallback_count": self.fallback_count,
