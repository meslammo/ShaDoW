...
class PersistentMemory:
    ...
    def _read(self)->list[MemoryItem]:
        ...
        for line in self.path.read_text(encoding='utf-8').splitlines():
            try:
                d=json.loads(line); d['tags']=tuple(d.get('tags',())); out.append(MemoryItem(**d))
    ...
    def search(self,query:str,limit:int=10)->list[MemoryItem]:
        terms={x.lower().strip('.,!?؟،؛:()[]{}') for x in query.split() if x.strip()}; scored=[]
        for item in self._read():
            hay=(item.text+' '+' '.join(item.tags)).lower(); score=sum(1 for t in terms if t and t in hay)
    ...
    def delete(self,item_id:str)->bool:
        with self._lock:
            old=self._read(); items=[x for x in old if x.id!=item_id]
            if len(items)==len(old): return False
