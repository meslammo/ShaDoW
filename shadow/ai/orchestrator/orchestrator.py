...
class AIOrchestrator:
    ...
    def _openai(self, request: str, key: str, profile: TaskProfile, context: Dict[str, Any],
                ...
                confirmed_actions: List[str], model: Optional[str] = None) -> Dict[str, Any]:
        model = (model or os.getenv("SHADOW_MODEL", profile.model)).strip() or profile.model
    ...
    def _post(payload: Dict[str, Any], key: str) -> Dict[str, Any]:
        req = Request("https://api.openai.com/v1/responses", data=json.dumps(payload).encode("utf-8"), headers={"Authorization":f"Bearer {key}","Content-Type":"application/json"}, method="POST")
        with urlopen(req, timeout=45) as response: return json.loads(response.read().decode("utf-8"))
