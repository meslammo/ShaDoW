...
_STAGE_NAMES = ("understand","model_route","voice_multimodal","memory","web_discovery","github_development","capability_execution","agent_loop","security_approval","execute","verify","deliver")
...
class SuperNiceMasterPipeline:
    ...
    def _classify(self, request):
        ...
        return {"intent":intent,"selected_core":selected,"fresh":fresh,"development":development,"voice":voice,"image":image,"risky":risky,"device_control":False}
    ...
    def run(self, request, *, authenticated=False, confirmed=False, context=None, execute_selected=True):
        ...
        stages.append(_stage("voice_multimodal","adapter_ready",requested=modalities,voice_transport="client_speech_or_cloud_stt",image_transport="uploaded_image_or_client_camera",device_control=False,note="Voice/image are client transports only; Shadow does not control the host device."))
        ...
        stages.append(_stage("capability_execution","software_only",selected_core=cls["selected_core"],device_control=False,external_devices=False))
