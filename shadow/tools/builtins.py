...
def _calc_node(node: ast.AST) -> float:
    ...
    if isinstance(node, ast.UnaryOp) and type(node.op) in _UN:
        return _UN[type(node.op)](_calc_node(node.operand))
...
def build_builtin_registry(*, memory: Any = None, devices: Any = None) -> ToolRegistry:
    ...
    registry.register(ToolSpec("calculator", "Evaluate safe numeric arithmetic.", calculate,
                               requires_confirmation=False, category="utility",
                               parameters={"type":"object","properties":{"expression":{"type":"string"}},"required":["expression"],"additionalProperties":False}))
    registry.register(ToolSpec("time.now", "Return the local system time and date.", current_time,
                               requires_confirmation=False, category="utility",
                               parameters={"type":"object","properties":{},"additionalProperties":False}))
    if memory is not None:
        registry.register(ToolSpec("memory.search", "Search SHADOW persistent memory.",
                                   lambda query, limit=5: [m.text for m in memory.search(query, int(limit))],
                                   requires_confirmation=False, category="memory",
    ...
    if devices is not None:
        registry.register(ToolSpec("device.status", "Return registered device status.",
                                   lambda: devices.snapshot(), requires_confirmation=False, category="device",
                                   parameters={"type":"object","properties":{},"additionalProperties":False}))
    return registry
