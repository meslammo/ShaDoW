...
const systemPrompt = String(process.env.SHADOW_SYSTEM_PROMPT || [
  ...
  'When the user asks to create or design an image, tell the client to use the image-generation endpoint rather than pretending an image exists.',
...
app.get('/health', (_req, res) => res.json({ ok: true, service: 'shadow-cloud', online: Boolean(apiKey), model, image_model: imageModel, web_search: true }));
...
app.post('/v1/chat', rateLimit, async (req, res) => {
  ...
  try {
    ...
    if (!upstream.ok) {
      console.error('OpenAI error', upstream.status, JSON.stringify(body).slice(0, 3000));
...
app.post('/v1/images', rateLimit, async (req, res) => {
  ...
  try {
    const upstream = await fetch('https://api.openai.com/v1/images/generations', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: imageModel, prompt, size: '1024x1024', output_format: 'png' }),
      signal: AbortSignal.timeout(120_000)
    });
    const body = await upstream.json().catch(() => ({}));
    if (!upstream.ok) {
      console.error('Image API error', upstream.status, JSON.stringify(body).slice(0, 3000));
      return res.status(502).json({ ok: false, error: 'image_upstream_error' });
    }
    ...
    return res.status(502).json({ ok: false, error: 'image_unreachable' });
  }
});
...
app.listen(port, '0.0.0.0', () => console.log(`SHADOW cloud backend listening on ${port}; model=${model}; image=${imageModel}; configured=${Boolean(apiKey)}`));
