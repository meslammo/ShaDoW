...
async function gh(path, options={}){
  ...
  const r=await fetch(`${GITHUB_API}${path}`,{...options,headers:{...headers(),...(options.headers||{})},signal:AbortSignal.timeout(30_000)});
  const body=await r.json().catch(()=>({}));
  if(!r.ok) throw new Error(`github_${r.status}:${body?.message||'request_failed'}`);
  return body;
}

export async function applyFiles({branch='shadow-agent-work',message='MOD-49: Development Agent change',files=[]}){
  if(!configured()) throw new Error('github_write_not_configured');
  ...
  for(const f of files){
    ...
    let current=null;
    try{current=await gh(`/repos/${REPO}/contents/${path}?ref=${encodeURIComponent(branch)}`);}catch(e){if(!String(e.message).startsWith('github_404:')) throw e;}
    const payload={message,content:Buffer.from(f.content,'utf8').toString('base64'),branch};
    if(current?.sha) payload.sha=current.sha;
    const saved=await gh(`/repos/${REPO}/contents/${path}`,{method:'PUT',body:JSON.stringify(payload)});
    result.push({path:f.path,commit_sha:saved?.commit?.sha||null,created:!current?.sha});
  }
  return {repo:REPO,branch,files:result};
}

export function status(){return {configured,repo:REPO,allowed_prefixes:ALLOWED_PREFIXES};}
